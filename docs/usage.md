# nf-core/neoprednf: Usage

## :warning: Please read this documentation on the nf-core website: [https://nf-co.re/neoprednf/usage](https://nf-co.re/neoprednf/usage)
...
```console
nextflow run nf-core/neoprednf --input samplesheet.csv --genome GRCh37 -profile docker
```
...
```console
nextflow pull nf-core/neoprednf
```
...
First, go to the [nf-core/neoprednf releases page](https://github.com/nf-core/neoprednf/releases) and find the latest version number - numeric only (eg. `1.3.1`). Then specify this when running the pipeline with `-r` (one hyphen) - eg. `-r 1.3.1`.
